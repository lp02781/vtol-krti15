...
{	
	...
	ros::NodeHandle n;  
	ros::Publisher ros_pub = n.advertise<geometry_msgs::Point>("krti15/topic_point", 1000);
	...
	Mat imgCircle = Mat::zeros( imgTmp.size(), CV_8UC3 );
	Mat imgDetection;
	while(n.ok()){
		...
		imgCircle = Mat::zeros( imgTmp.size(), CV_8UC3 ); 
