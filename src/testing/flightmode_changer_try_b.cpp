	...
	while (ros::ok()){
		...
		if(rc_in_channel_7 > channel_7_mid){
			if(flightmode_data == 0 && last_flightmode_data == 1){
				...
				rc_override_msg.channels[4] = 1412;			// LOITER, Flight Mode 3, Channel 5
				pub_rc_override.publish(rc_override_msg);
				ROS_INFO("LOITER");
				
				usleep(2000000);
			
				rc_override_msg.channels[4] = 1525;			// AUTO, Flight Mode 4, Channel 5
				pub_rc_override.publish(rc_override_msg);
				ROS_INFO("AUTO");
				...
				rc_override_msg.channels[4] = 1234;			// GUIDED, Flight Mode 2, Channel 5
				pub_rc_override.publish(rc_override_msg);
				ROS_INFO("GUIDED");
