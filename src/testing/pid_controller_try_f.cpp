...
int main(int argc, char **argv){
	...
	float const_cv_local = 0.019003268;
	float KpH = 0.0015 ;
	float KiH = 0 ;
	float KdH = 0 ;
	...
	while (ros::ok()){
		...
		if ( x_pos_flag == 1 && y_pos_flag == 1 && state_char == 'G'){
			...
			timer_qc.start();
			#elif QUAD == GARUDA			
			timer_qc.setPeriod(ros::Duration(time_out_second*6));
			timer_qc.start();
			#endif
		...
		quad_pos.header.stamp = ros::Time::now();
		quad_pos.header.frame_id = "1";
		quad_pos.pose.position.x = pos_x + local_pos_cv_x;
		quad_pos.pose.position.y = pos_y + local_pos_cv_y;
